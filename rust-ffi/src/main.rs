...
fn main() {
    ...
    for i in 1..100{
        let cipher_test = encrypt(pk.clone(), message.to_bigint().to_string(), random.to_string());
    }
    ...
    start = Instant::now();
    for i in 1..100{
        let proof_test = cl_ecc_prove(pk.clone(), cipher.clone(), commit_str.clone(), message.to_bigint().to_string(), random.to_string());
    }
    end = Instant::now();
