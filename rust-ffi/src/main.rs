...
extern "C" {
    ...
    pub fn encrypt_cpp(pk_str: *const c_char, message: *const c_char) -> *const c_char;
...
pub fn encrypt(pk: String, message: String) -> String{
    let pk_str = CString::new(pk).unwrap();
    let m_str = CString::new(message).unwrap();
    unsafe{
        return c_char_decode(encrypt_cpp(pk_str.as_ptr(), m_str.as_ptr()));
    }
}
...
pub fn encrypt_and_decrypt_test_rust(message: String){
    ...
    unsafe{
        let res = c_char_decode(encrypt_and_decrypt_test(m_prt));
        println!("{res}");
    }
}
...
fn main() {
    ...
    println!("pk_rust: {pk}");
    let cipher = encrypt(pk.clone(), message.to_string());
